...
&emsp;&emsp;主干网络为ResNet50-vd-FPN的配置文件为[mask_rcnn_r50_vd_fpn.yml]()，该配置文件的部分参数是针对同时使用8块显卡训练COCO数据集所设置的，运行前请根据实际情况调整这些参数：

* max_iters：表示模型的最大迭代次数
  * COCO训练集：8块显卡训练时最大迭代次数设置为360000次，当显卡数量为`n(n<=8)`时，最大迭代次数为`360000*(8/n)`次。例如，4块显卡训练时设置为`360000*4`次，1块显卡训练时设置为`360000*8`次，
  * 自定义数据集一共
